...
export class GearScrollbar extends LitElement {
  ...
  connectedCallback() {
    ...
    if (this.parentElement) {
      ...
      const scrollHeight = Number(this.parentElement!.scrollHeight);
      const multiple = 36;
      for (let i = multiple; i < scrollHeight; i += multiple) {
        this.points.push(`${i}`);
      }
  ...
  render() {
    return html`
      <div style="height: ${this.height}px; position: absolute">
        <svg
          version="1.0"
        ...
        >
          ...
          <g style="clip-path: url(#myClip);">
            <g transform="translate(98, 90), scale(0.5, 0.5)">
              <g style="transform: rotate(${this.angle / 2}deg)" stroke="none">
                <path
            ...
            <g style="transform:translate(0, -${this.angle / 2}px)">
              ${this.points.map(
                (p) =>
